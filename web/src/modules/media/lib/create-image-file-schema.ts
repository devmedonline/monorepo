...
export const createImageFileSchema = (
  options: CreateImageFileSchemaOptions = DEFAULT_OPTIONS
) => {
  ...
  const acceptedFileTypesString = listFormat.format(
    options.acceptedFileTypes ?? DEFAULT_OPTIONS.acceptedFileTypes
  );
  ...
  const isInAcceptedFileTypes = (fileList: File[] | null) => {
    ...
    return Array.from(fileList).every((file) =>
      ACCEPTED_IMAGE_FILE_TYPES.includes(file.type)
    );
  };
